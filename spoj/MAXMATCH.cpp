...
//-----------------------------------------------------------------------------------------------------------
    ...
    cin>>s;
    int n=s.size();
    vector<int> a(2*n+1), a_1(n+1), b(2*n+1), b_1(n+1), c(2*n+1),c_1(n+1);
    for(int i=0;i<n;i++)
    {
        if(s[i]=='a')
        {
    ...
    auto res1=multiply(a, a_1);
    auto res2=multiply(b, b_1);
    auto res3=multiply(c, c_1);
    a.resize(3*n+1);
    for(int i=2*n+1;i<3*n;i++)
        a[i]=res1[i]+res2[i]+res3[i];
    int mx=0;
    for(int i=2*n+1;i<3*n;i++)
        mx=max(a[i],mx);
    cout<<mx<<endl;
